    ...
    for attempt in range(retries):
        try:
            ...
            if r.status_code == 400:
                return None  # ignora closed/under_review/bad_request
            if not r.ok:
                if r.status_code not in (401, 429):
                    print(f"[ML API] {r.status_code} em {path}: {r.text[:100]}", flush=True)
                return None
            return r.json()
  ...
  try {
    ...
    _polling = setInterval(async () => {
      ...
      if (s.status === 'ready') {
        ...
        let dados; try { dados = await res.json(); } catch(e) { return; }
        if (!dados) return;
        dadosGlobais = dados;
        renderizar(dados);
        btn.disabled = false;
